...
export default function Settings() {
  ...
  const getProfile = async () => {
    try {
      ...
      setProfile(data);
      setDisplayName(data.username || data.full_name || ""); // Fallback to username or full_name if display name is empty
      setAvatarUrl(data.avatar_url);
    } catch (error: any) {
      toast.error(error.message);
    }
  ...
  const handleDisplayNameUpdate = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from("profiles")
        .update({ username: displayName })  // Update both username and full_name if necessary
        .eq("id", profile.id);
